...
import io.techery.snapper.executor.SimpleExecutorService;
...
public class DataCollection<T extends Indexable> extends DataSet<T> implements Storage.UpdateCallback<T> {
    ...
    public IProjection.Builder<T> projection() {
        return new ProjectionBuilder<T>(this);
    }
    ...
    public void removeAll(final List<T> items) {
        storage.removeAll(ListUtils.map(items, new Function1<T, ItemRef<T>>() {
            @Override
            public ItemRef<T> apply(T t) {
                return ItemRef.make(t);
            }
